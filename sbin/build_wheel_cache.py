...
def _pip_install(pyver, pkg_name, quiet=False):
    """ Install package via pip """
    return _os_cmd(
        ['pip{0}'.format(pyver), 'install', '--upgrade',]+
        (['--quiet'] if quiet else [])+['--force-reinstall', pkg_name], quiet
    )
    ...
    for pyver in pyvers:
        ...
        for line in lines:
            ...
            if 'scipy' in line:
                ...
                retcode = _pip_install(pyver, numpy_line.strip(), quiet)
                if retcode and exitfirst:
                    sys.exit(1)
            retcode = _os_cmd(
